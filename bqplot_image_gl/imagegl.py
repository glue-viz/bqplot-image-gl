...
DEFAULT_IMAGE_DATA_COMPRESSION = os.environ.get(
    "BQPLOT_IMAGE_GL_IMAGE_DATA_COMPRESSION", "none"
)
...
class ImageGL(bqplot.Mark):
    ...
    compression = Unicode(DEFAULT_IMAGE_DATA_COMPRESSION, allow_none=True).tag(
        sync=True
    )
    interpolation = Unicode("nearest", allow_none=True).tag(sync=True)
    opacity = Float(1.0).tag(sync=True)
    x = (
        Array(default_value=(0, 1))
        .tag(
            sync=True,
            scaled=True,
            rtype="Number",
...
double_list_array_serialization = dict(
    to_json=double_list_array_to_json, from_json=double_list_array_from_json
)
...
class Contour(bqplot.Mark):
    ...
    image = Instance(ImageGL, allow_none=True).tag(
    ...
    color = (
        widgets.Color(None, allow_none=True)
        | List(widgets.Color(None, allow_none=True))
    ).tag(sync=True)
    scales_metadata = Dict(
        {
            "x": {"orientation": "horizontal", "dimension": "x"},
